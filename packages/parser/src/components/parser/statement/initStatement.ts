...
export default class InitStatement extends Statement {
  ...
  getStatement(): ASTNode {
    this._tokenExecutor.eatTokenAndForwardLookahead(TokenTypes.HI_anna_TYPE);

    this._tokenExecutor.eatOptionalSemiColonToken();

    const body =
      this._tokenExecutor.getLookahead()?.type !== TokenTypes.BYE_anna_TYPE
        ? this._statementList.getStatementList(TokenTypes.BYE_anna_TYPE)
        : [];

    this._tokenExecutor.eatTokenAndForwardLookahead(TokenTypes.BYE_anna_TYPE);
